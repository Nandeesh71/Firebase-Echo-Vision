...
const CollaborateModal: React.FC<CollaborateModalProps> = ({ isOpen, onClose }) => {
    ...
    const [isOpening, setIsOpening] = useState(false); // Controls exit animation
    ...
    const handleCardClick = () => {
        ...
        setIsOpening(true); // Trigger animation state
        setHasCelebrated(true);
        fireConfetti(); // Fire confetti if not already

        // Delay redirect to match CSS transition-duration (2s)
        setTimeout(() => {
...
[Your Name]`);
            ...
            // Optional: Close modal after redirect (or let user close it)
            // onClose(); 
        }, 2000);
    };
    ...
    return (
        ...
        >
            ...
            >
                <div
                    className={`card-wrapper cursor-pointer group ${isOpening ? 'is-opening' : ''}`}
                    onMouseEnter={handleHover}
                    onClick={handleCardClick}
                >
                    <div className="relative bg-zinc-900 w-[90vw] max-w-[500px] aspect-[16/10] transition-all duration-700 flex items-center justify-center shadow-2xl rounded-md overflow-visible mx-auto">

                        {/* White Content Letter */}
                        <div className={`card-animate flex flex-col items-center py-4 px-4 sm:py-6 sm:px-6 justify-between bg-white w-[92%] h-[90%] absolute bottom-0 mb-3 left-0 right-0 mx-auto ${isOpening ? '-translate-y-[65%] scale-[1.02]' : 'group-hover:-translate-y-[65%] group-hover:scale-[1.02]'} rounded-lg shadow-lg text-center border border-slate-100`}>

                            {/* Header Content */}
                            <div className="flex flex-col items-center gap-1.5 sm:gap-2 mt-1 w-full max-w-[95%] sm:max-w-[90%]">
                                <h3 className="text-base sm:text-lg md:text-xl font-bold text-slate-900 tracking-tight">
                                    Thank You for Reaching Out
                                </h3>
                                <div className="h-px w-12 bg-slate-200 my-0.5 sm:my-1"></div>
                                <p className="text-[11px] sm:text-xs md:text-sm text-slate-600 leading-relaxed font-medium line-clamp-4 sm:line-clamp-none">
                            ...
                            <div className="flex flex-col items-center gap-1 w-full">
                                <p className="text-[10px] sm:text-[11px] text-slate-400 italic font-medium">
                                    Together, we can create a more accessible and inclusive world.
                                </p>
                                <div className="w-full border-t border-slate-100 pt-2 sm:pt-2.5">
                        ...
                        {/* Seal */}
                        <button className={`seal w-10 h-10 sm:w-12 sm:h-12 rounded-full z-40 text-[10px] sm:text-xs flex items-center justify-center font-bold text-white bg-slate-900 transition-all duration-700 border-[3px] border-zinc-800 shadow-xl ${isOpening ? 'opacity-0 scale-50' : 'group-hover:opacity-0 group-hover:scale-50'}`}>
                            EV
                        </button>
